...
export const Login = () => {
    ...
    const signUpSchema = Yup.object({
        username: Yup
            ...
            .min(6, "User ID must be atleast 5 Characters")
            .required("User ID is required")
            .matches("^(?=[a-zA-Z0-9._]{8,20}$)(?!.*[_.]{2})[^_.].*[^_.]$", "Please write Alphanumeric UserId without spaces"),
        password: Yup
            .string()
            .required("Please enter password")
    ...
    const formik = useFormik({
        ...
        onSubmit: (values) => {
            loginFormMode ? loginAuth(values) : signUp(values)
        }
    })
                ...
                <form onSubmit={formik.handleSubmit}>
                    <div className={`loginForm ${loginFormMode ? `mt-[2.75rem]` : `mt-[1rem]`} flex flex-col gap-[0.81rem] w-full`}>
                        <div className="formControl flex flex-col justify-center items-start gap-[0.81rem] ">
                            ...
                            <input autocomplete="username" onChange={formik.handleChange} type="text" onBlur={formik.handleBlur} value={formik.values.username} name="username" id="username" className="w-full h-[2.1875rem] disabled:bg-[#0000003d] border-black border-2 text-[#A0616A] text-[0.75rem] font-[700] px-[0.69rem] py-[0.63rem]" />
                        ...
                        {!loginFormMode && <div className="formControl  flex justify-center items-start gap-[0.81rem] ">
                            <div className="names text-sm">
                                ...
                                <input autocomplete="first-name" onChange={formik.handleChange} type="text" onBlur={formik.handleBlur} value={formik.values.firstName} name="firstName" id="firstName" className="w-full h-[2.1875rem] disabled:bg-[#0000003d] border-black border-2 text-[#A0616A] text-[0.75rem] font-[700] px-[0.69rem] py-[0.63rem]" />
                            ...
                            <div className="names">
                                ...
                                <input autocomplete="last-name" onChange={formik.handleChange} type="text onBlur={formik.handleBlur}" value={formik.values.lastname} name="lastname" id="lastname" className="w-full h-[2.1875rem] disabled:bg-[#0000003d] border-black border-2 text-[#A0616A] text-[0.75rem] font-[700] px-[0.69rem] py-[0.63rem]" />
                                {formik.errors.lastname && formik.touched.lastname ? <p className="text-red-700 text-xs font-bold">{formik.errors.lastname}</p> : null}
                            </div>
                        </div>}
                        <div className="formControl flex flex-col justify-center items-start gap-[0.81rem] w-full ">
                            <div className="label text-sm">Password</div>
                            <input autocomplete={loginFormMode ? "current-password" : "new-password"} onChange={formik.handleChange} value={formik.values.password} onBlur={formik.handleBlur} type="password" name="password" id="password" className="w-full h-[2.1875rem] disabled:bg-#0000003d border-black border-2 text-[#A0616A] text-[0.75rem] font-[700] px-[0.69rem] py-[0.63rem]" />
                        ...
                        {!loginFormMode && <div className="formControl flex flex-col justify-center items-start gap-[0.81rem] w-full ">
                            ...
                            <input onChange={formik.handleChange} value={formik.values.confirm_password} onBlur={formik.handleBlur} type="password" name="confirm_password" id="confirm_password" className="w-full h-[2.1875rem] disabled:bg-#0000003d border-black border-2 text-[#A0616A] text-[0.75rem] font-[700] px-[0.69rem] py-[0.63rem]" />
                    ...
                    <div className="login__footer mt-[1.5rem] w-full">
                        ...
                        <div className="action__buttons flex flex-col w-full gap-[0.56rem] mt-[0.56rem]">
                            <button type="submit" className="bg-[#FFB8B8] transition-transform text-white font-bold   hover:scale-105 shadow-[0px_10px_12px_-6px_#6c63FF] h-[1.875rem]">Join The Buzz</button>
                            <button onClick={() => {
                                guestLogin()
