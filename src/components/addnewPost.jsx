...
function AddNewPost({ postId }) {
    const { setShowNewPost, addNewPost } = usePostContext();
    const [mode, setMode] = useState("NEW__POST")
    if (postId) {
        setMode("EDIT__POST")
    }
    ...
    const formik = useFormik({
        ...
        onSubmit: (values) => {
            ...
            const response = addNewPost(postValues)
            if (response) {
                formik.resetForm()
    ...
    return (
        <div className="z-10 absolute  rounded-lg top-0 left-0 w-full h-full backdrop-filter backdrop-blur-lg flex items-center justify-center">
            <button onClick={() => setShowNewPost(false)} className="absolute right-[2%] top-[3%]">
                ...
                <div className="add__new__post__box w-[50rem] shadow-lg  bg-white flex flex-col items-center justify-center gap-[0.69rem] px-[1.44rem] py-[1.06rem]">
                    ...
                    <footer className="new__post__footer flex  items-center w-full gap-2" >
                        <div className="iconLIst rounded-[5px] bg-[#f1f1f1] shadow-[0px_4px_8px_-4px_rgba(0,0,0,0.25)] h-[1.875rem] w-[5.0625rem] grid grid-cols-2 place-content-center">
                            {mode === "NEW__POST" && <button type="button" onClick={imageClickHandler} className="text-center grid place-content-center text-[#6C63FF] hover:text-white">
                        ...
                        <button
                            ...
                            type="submit"
                            className="button disabled:bg-slate-200 disabled:text-slate-300 disabled:shadow-none disabled:transition-none transition-all text-white capitalize text-cenbutton py-2  w-[7.4375rem]  shadow-md shadow-[#6C63FF] bg-[#6C63FF] hover:bg-[#514bc5]">Post It!</button>
                        <button
                            type="reset"
                            className="button transition-all text-white text-cenbutton py-2 w-[7.4375rem] bg-red-500 shadow-md shadow-red-400 hover:bg-red-600">Reset</button>
                    </footer>
                </div>
            </form>
        </div>
